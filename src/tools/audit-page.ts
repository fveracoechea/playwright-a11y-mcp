...
export function auditPageTool(mcp: McpServer) {
  mcp.registerTool(
    "a11y_audit_page",
    {
      ...
      description:
        "Perform an WCAG 2.1 AA accessibility audit on a given webpage URL",
    ...
    async function ({ url }) {
      try {
        ...
        page.on(
          "console",
          (msg) => console.log(`PLAYWRIGHT LOG`, url, msg.text()),
        );
        ...
        throw new McpError(
          ...
          `${
            e instanceof Error ? e.message : "Error auditing webpage"
          } - ${url}`,
        );
      }
    },
